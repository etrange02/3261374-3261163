	...
	public static void changeMethodeTr(CtClass cc,CtMethod cm) throws CannotCompileException, NotFoundException{
			...
			cm.instrument(new ExprEditor() {
				public void edit(Handler ha) throws CannotCompileException{
					ha.insertBefore("fr.upmc.aladyn.transactionables.TransacPool.Get().restore(Thread.currentThread().getId()); System.out.println(\"Je suis dans le catch !\");");
				}
			});
			
			CtClass excep = ClassPool.getDefault().get("java.lang.Exception");
			cm.addCatch("System.out.println(\"Je suis le new catch !\");return;",excep );
...
//			cc.addMethod(nm);
	...
	public static void changeSet(CtClass cc, CtMethod m) throws CannotCompileException {
